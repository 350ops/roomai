...
export const TabButton = forwardRef<View, TabButtonProps>(
  ({ icon, avatar, children, isFocused, onPress, customContent, labelAnimated = true, hasBadge = false, labelHidden = false, ...props }, ref) => {
        ...
        return (
          <View className="relative">
            <View className={`w-full relative ${isFocused ? 'opacity-100' : 'opacity-40'}`}>
              ...
              <Icon name={icon} size={22} strokeWidth={isFocused ? 2.5 : 2} color={isFocused ? colors.text : colors.icon} />
            </View>
            {hasBadge && (
      ...
      <Pressable
        ...
        <View className="flex-col items-center justify-center pt-4 pb-0 w-full relative">
          ...
          {labelAnimated ? (
            ...
            >
              <Text className={`text-[9px] mt-px text-text`}>
                {children}
              </Text>
            </Animated.View>
          ) : (
            <ThemedText className={`text-[9px] mt-px ${labelHidden ? '!text-transparent' : ''}`}>
              {children}
            </ThemedText>
          )}
        </View>
